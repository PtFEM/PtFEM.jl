...
if :eq_nodal_forces_and_moments in keys(data)
  ...
  for t in eqfm
    ...
    for i in 1:k
      fm_dt[t[1], i + 2] = round(vals[i] - t[2][i], 2)
    end
  end
end
...
if VERSION.minor < 6
  ...
  N = vcat(
    convert(Array, fm_dt[:normal_force_1_corrected])[1],
    convert(Array, fm_dt[:normal_force_2_corrected])
  )
    
  p = Vector{Plots.Plot{Plots.GRBackend}}(2)
  titles = ["EEM fig1.1 u(x)", "EEM fig1.1 N(x)"]
  ...
  p[2] = plot(u, x, xlim=(0.0, 3.0), yflip=true,
