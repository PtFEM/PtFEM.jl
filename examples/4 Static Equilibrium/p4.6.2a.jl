...
if VERSION.minor > 5
  ...
  p[1] = plot(buckling_df[:translation], ylim=(-0.1, 0.1),
    xlabel="node", ylabel="y translation [m]", color=:blue,
    marker=(:circle,1,0.1,stroke(1,:black)),
    title=titles[1], leg=false)
  p[2] = plot(buckling_df[:rotation], ylim=(-1.0, 1.0),
    xlabel="node", ylabel="rotation [radians]", color=:red,
    marker=(:circle,1,0.1,stroke(1,:black)),
    title=titles[2], leg=false)

  plot(p..., layout=(2, 1))
  savefig(ProjDir*"/p4.6.2a.png")
